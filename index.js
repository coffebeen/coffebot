...
const AUTOROLE_ID = "1490815818492149901";
// ─────────────────────────────────────────────────────────────────────────────
...
async function createTempRoom(member, lobbyChannel) {
  ...
  try {
    ...
    console.error("Failed to create temp room:", err);
  }
}
...
client.on("guildMemberAdd", (member) => {
  if (features.welcome) {
    ...
    if (role) {
      member.roles
        .add(role)
        .catch((err) => console.error("Failed to add auto-role:", err));
    } else {
      console.warn(
        `⚠️  Auto-role not found! Make sure AUTOROLE_ID is set correctly at the top of bot.js`
      );
    }
  }
